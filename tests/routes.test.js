...
describe("GET /api/users", () => {
    ...
    test("should return a description to a single user", async () => {
      const response = await request(app).get("/api/users/user/63fc0091ca93b9be05b47123");
...
describe("DELETE /api/users/delete/:id", ()=>{
  ...
  test("should delete a user", async()=>{
      const response = await request(app).delete("/api/users/delete/63fbfdafd57f4c320768941d")//Esse teste só ira dar certo na primeira, pois na segunda tentativa o usuário já vai ter sido deletado
      expect(response.statusCode).toBe(200)
  })

  test("should return a status code 404", async()=>{
    const response = await request(app).delete("/api/users/delete/63fbfdafd57f4c3skas");
    if(!response){
...
describe("PATCH /api/users/:id", ()=>{
  ...
  test("should update a user", async()=>{
    const response = await request(app).patch("/api/users/update/63fc00195b2ac54345d1781d").send({
      name : "Vitor",
      dob: "1997-04-09",
      address: "Rua certa",
